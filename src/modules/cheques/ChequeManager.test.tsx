...
const mocks = vi.hoisted(() => ({ command: vi.fn(), get: vi.fn(), upload: vi.fn(), accounts: vi.fn() }));
vi.mock('./api', () => ({ chequesApi: { byPayment: mocks.get, command: mocks.command } }));
vi.mock('@/modules/auth/store/auth.context', () => ({ useAuth: () => ({ user: { userId: 1 }, hasRole: (roles: string[]) => roles.includes('JEFA_CUENTAS') }) }));
...
beforeEach(() => { vi.clearAllMocks(); mocks.get.mockResolvedValue(cheque); mocks.upload.mockResolvedValue({ downloadUrl: 'https://example.com/deposito' }); mocks.accounts.mockResolvedValue([{ id: 8, banco: 'Bajío', titular: 'Empresa', numeroCuenta: '123456', activo: true }]); });
