...
export function OperationalHealth({ period, summary, isLoading }: { period: PeriodRange; summary: GerenteDashboardSummary; isLoading: boolean }) {
  ...
  return <>
    ...
    <DashboardSection title="Carga por área" description="Cantidad de tareas, no operaciones únicas; una operación puede requerir acciones en más de una cola.">
      <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="bg-slate-100 text-left text-xs uppercase text-slate-500"><th className="px-4 py-3">Área</th><th className="px-4 py-3">Tareas pendientes</th><th className="px-4 py-3">Estado</th><th className="px-4 py-3">Detalle</th></tr></thead><tbody>
        {[{ label: 'Cajas', count: cash, role: 'JEFA_CAJAS' }, { label: 'Cuentas', count: accounts, role: 'JEFA_CUENTAS' }].map((area) => <tr key={area.label} className="border-t border-slate-100"><td className="px-4 py-3 font-medium">{area.label}</td><td className="px-4 py-3 tabular-nums">{area.count ?? 'Cargando…'}</td><td className="px-4 py-3">{area.count === null ? 'Consultando' : area.count === 0 ? 'Sin pendientes' : 'Requiere atención'}</td><td className="px-4 py-3"><button className="font-medium text-blue-600 hover:underline" onClick={() => navigate(`${paths.teamPending}?supervisedRole=${area.role}&startDate=${period.startDate}&endDate=${period.endDate}&dateFilter=`)}>Supervisar</button></td></tr>)}
      </tbody></table></div>
